...
export class MerkleTree {
  ...
  private static async hashChildrenAndLinkToParent(
    levelLeaves: TreeNode[]
  ): Promise<TreeNode[]> {
  ...
  public static createFromStorageString(ss: string): MerkleTree {
    ...
    for (let lineIndex = 1; lineIndex < lines.length; lineIndex++) {
      ...
      for (let rowIndex = 0; rowIndex < currRow.length; rowIndex++) {
        ...
        let lChild = new TreeNode(
          BigInt(vals[2 * rowIndex]),
          undefined,
          undefined,
          parent
        );
        let rChild = new TreeNode(
          BigInt(vals[2 * rowIndex + 1]),
          undefined,
          undefined,
          parent
        );
