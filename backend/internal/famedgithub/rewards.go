...
func (c *client) GetRewardsByOwner(ctx context.Context, owner string) ([]byte, error) {
	var bytes []byte
	_, err := c.execute(ctx, http.MethodGet, fmt.Sprintf("/famed/owners/%s/rewards", owner), nil, &bytes)
	return bytes, err
}
...
func (c *client) GetRewardsByContributor(ctx context.Context, contributor string) ([]byte, error) {
	var bytes []byte
	_, err := c.execute(ctx, http.MethodGet, fmt.Sprintf("/famed/contributors/%s/rewards", contributor), nil, &bytes)
	return bytes, err
}
